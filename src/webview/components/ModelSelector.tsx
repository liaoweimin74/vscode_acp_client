...
import { flattenSelectOptions, groupModelsByFamily, getModelLevel, isGroupedOptions } from "@shared/utils";
...
export function ModelSelector({ modelOption, currentValue, onSelect, variant, onClose }: ModelSelectorProps) {
	...
	const allItems: FlatItem[] = modelGroups.length > 0
		? modelGroups.flatMap((g) => g.options.map((o) => ({
				value: o.value,
				label: g.label + (g.options.length > 1 ? ` – ${getModelLevel(o.name) ?? "Default"}` : ""),
				group: g.family,
			})))
		...
		? modelGroups
				.map((g) => ({
					...
					options: g.options.filter((o) => {
						const label = g.label + (g.options.length > 1 ? ` – ${getModelLevel(o.name) ?? ""}` : "");
	...
	const handleSelectFamily = useCallback((group: ModelGroup) => {
		const target = group.options.find((o) => o.value === currentValue)
		...
		return (
			<div className="acp-model-selector">
				<div className="acp-model-selector__search">
					<svg width="12" height="12" viewBox="0 0 16 16" fill="none" className="acp-model-selector__search-icon">
				...
				{currentGroup && currentGroup.options.length > 1 && !query && currentGroup.options.some((o) => getModelLevel(o.name)) && (
				...
				<div className="acp-model-selector__list" ref={listRef}>
					{groupsToRender.length === 0 && modelGroups.length === 0 && flatOptions.length > 0
						...
						: groupsToRender.length === 0 && (
								<div className="acp-model-selector__empty">No models found</div>
							)
					}
					{modelGroups.length > 0 && groupsToRender.map((group, i) => {
									...
									return (
										...
										>
											...
											{item.value === currentValue && (
												<svg width="14" height="14" viewBox="0 0 16 16" fill="none" className="acp-popup__check" role="img" aria-label="Current model">
